...
class Cidades{
	...
	public function contador(){
        $query = "SELECT count(*) FROM `cidades` WHERE 1";
    ...
    public function contadorPesquisa($nome){
        $query = "SELECT count(*) FROM `cidades` WHERE `nome` LIKE '%" . $nome . "%'";
        $stmt = $this->conn->prepare($query);
    ...
    public function paginacao($limite, $offset){
        try {
            $query = "SELECT * FROM `cidades` WHERE 1 ORDER BY `nome` ASC LIMIT :limite OFFSET :offset";
            $stmt = $this->conn->prepare($query);
    ...
    public function paginacaoPesquisa($nome, $limite, $offset){
        try {
            $query = "SELECT * FROM `cidades` WHERE `nome` LIKE :nome LIMIT :limite OFFSET :offset";
            $stmt = $this->conn->prepare($query);
            $nome = '%' . $nome . '%';
